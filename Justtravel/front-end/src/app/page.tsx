...
export default function Home() {
  ...
  const toggleCardStatus = async (index: number, statusAtual: boolean) => {
    try {
      ...
      if (response.ok) {
        setCards(prev =>
          prev.map(card => card.id === index ? { ...card, status: !statusAtual } : card)
        )
      } else {
  ...
  let tarefasPendentesFiltroPorTipo: CardType[] = [...tarefasPendentesFiltro];

if (filtroTipo === 'data') {
  tarefasPendentesFiltroPorTipo.sort((a, b) => {
  ...
  tarefasPendentesFiltroPorTipo.sort((a, b) => {
    const prioridadeA = prioridades.indexOf(a.prioridade);
    const prioridadeB = prioridades.indexOf(b.prioridade);
    console.log(prioridadeA, prioridadeB);
    ...
    <>
      ...
      <main className="flex flex-col px-10 py-4 gap-8 ">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <SheetDemo
            CardsVencidos={tarefasConcluidas}
        ...
        <div className="flex justify-center items-center w-full">
          <div className="sm:flex sm:flex-wrap col items-center gap-4 pt-8">
            {tarefasPendentesFiltroPorTipo.map(card => (
